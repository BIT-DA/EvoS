...
class RotatedMNISTNetwork_for_EvoS(nn.Module):
    def __init__(self, args, num_input_channels, num_classes):
        super(RotatedMNISTNetwork_for_EvoS, self).__init__()
        self.args = args
        self.conv1 = nn.Conv2d(num_input_channels, 64, 3, 1, padding=1)
        self.conv2 = nn.Conv2d(64, 128, 3, stride=2, padding=1)
        self.conv3 = nn.Conv2d(128, 128, 3, 1, padding=1)
        ...
        self.domain_discriminator = DomainDiscriminator(in_feature=self.feature_dim,
                                                        hidden_size=self.args.hidden_discriminator).cuda()
