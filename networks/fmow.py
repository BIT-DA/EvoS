...
class FMoWNetwork(nn.Module):
    def __init__(self, args, weights=None, ssl_training=False):
        ...
        if args.dim_bottleneck_f is not None:
            ...
            self.enc = nn.Sequential(densenet121(pretrained=True).features, nn.ReLU(), nn.AdaptiveAvgPool2d(output_size=(1, 1)), nn.Flatten(), self.bottleneck)
            self.feature_dim = args.dim_bottleneck_f
        else:
            self.enc = nn.Sequential(densenet121(pretrained=True).features, nn.ReLU(), nn.AdaptiveAvgPool2d(output_size=(1, 1)), nn.Flatten())
...
class FMoWNetwork_for_EvoS(nn.Module):
    def __init__(self, args, weights=None):
        super(FMoWNetwork_for_EvoS, self).__init__()
        self.args = args
        self.num_classes = NUM_CLASSES
